// src/components/Result/EditResult.tsx
"use client";
...
import type { ResultDoc, ResultType } from "@/lib/types";
...
export default function EditResult({ initial }: { initial: ResultDoc }) {
    ...
    const [selectedBatch, setSelectedBatch] = useState<string>(initial.batch);
    const [selectedStudent, setSelectedStudent] = useState<string>("");
    const [resultType, setResultType] = useState<ResultType | "">(initial.resultType);
    const [examDate, setExamDate] = useState<string>(toDateInputValue(initial.examDate));
        ...
        (async () => {
            try {
                ...
                setSelectedStudent((prev) => {
                    if (prev) return prev;
                    const match = list.find((s) => s.studentId === initial.studentId);
                    return match ? match._id : prev;
                });
            } catch {
                setStudents([]);
            }
        })();
        // Only re-run when the batch changes; matching the initial student by
        // code is a one-time seed and shouldn't re-trigger on its own.
    ...
    async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
        ...
        const res = await fetch(`/api/results/${initial._id}`, {
            method: "PATCH",
                                ...
                                <SelectTrigger className="w-full">
                                    <SelectValue>
                                        {(v: string) => {
                                            ...
                                            return s ? `${s.name} (${s.studentId})` : "-- Select Student --";
                                        }}
                                    </SelectValue>
                                </SelectTrigger>
                                <SelectContent>
                                        ...
                                        <div className="space-y-2">
                                            ...
                                            >
                                                ...
                                                <SelectContent>
                                                    ...
                                                    {!filteredClasses.some((c) => c.name === sub.className) && sub.className && (
                                                        <SelectItem value={sub.className}>{sub.className}</SelectItem>
                                                    )}
                                                </SelectContent>
                                            </Select>
                                        </div>
