    ...
    try {
        ...
        if (body.studentId) {
            const s = await prisma.student.findFirst({
                where: { studentId: body.studentId, batch: { name: body.batchId }, isSuspended: false },
            });
            if (s) students = [s];
        } else {
            students = await prisma.student.findMany({
                where: { batch: { name: body.batchId }, isSuspended: false },
            });
        }
        ...
        for (const s of students) {
            ...
            const res = await sendBulkSms(phone, msg, settings.smsApiKey, body.senderId || settings.smsSenderId);
        ...
        console.error("POST /api/sms/send/student error:", error);
